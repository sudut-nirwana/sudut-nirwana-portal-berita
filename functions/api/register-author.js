export async function onRequestPost(context) {
    try {
        ...
        // Definisikan variabel default dan sanitasi slug
        ...
        // Simpan ke database D1 SQLite
        await db.prepare("INSERT INTO users (email, password_hash, role, name, avatar) VALUES (?, ?, ?, ?, ?)")
            .bind(email, passwordHash, authorRole, name, authorAvatar)
            .run();

        // Struktur Markdown lengkap dengan variabel slug dan role agar terbaca sempurna oleh Jekyll
        const markdownContent = `---
layout: author
name: ${JSON.stringify(name || '')}
slug: ${JSON.stringify(authorSlug)}
